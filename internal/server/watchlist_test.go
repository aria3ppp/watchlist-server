...
func TestHandleWatchlistGet(t *testing.T) {
	...
	e.Request(method, path).
		...
		JSON().
		Object().
		Equal(response.Paginated(config.Config.Pagination.Page.MinValue, config.Config.Pagination.PageSize.DefaultValue, watchlistItems, len(watchlistItems)))
}
