...
func (q paginationSortingQueryToQueryOptions) ToQueryOptions() query.Options {
	offset, limit := q.PaginationQuery.toOffsetLimit()
...
func (r PaginationQuery) Validate() error {
	return validation.ValidateStruct(
		...
		validation.Field(
			&r.Page,
			validation.Min(config.Config.Pagination.Page.MinValue),
		),
		validation.Field(
			&r.PageSize,
			validation.Min(config.Config.Pagination.PageSize.MinValue),
			validation.Max(config.Config.Pagination.PageSize.MaxValue),
		),
	)
}

func (p PaginationQuery) toOffsetLimit() (offset, limit int) {
