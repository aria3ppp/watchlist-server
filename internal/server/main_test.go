...
func setup(
	...
	if err != nil {
		log.Fatalf("server_test.setup: postgres.WithInstance error: %s", err)
	}
	...
	err = migrator.Up()
	if err != nil {
		log.Fatalf("server_test.setup: migrator.Up error: %s", err)
	}
	...
	if OptEnableLogger&opts != 0 {
		...
		if err != nil {
			log.Fatalf("server_test.setup: zap.NewDevelopment error: %s", err)
		}
	}
	...
	teardown = func() {
		...
		err = migrator.Drop()
		if err != nil {
			log.Fatalf("server_test.teardown: migrator.Drop error: %s", err)
		}
		// close server
		testServer.Close()
	}
...
func TestMain(m *testing.M) {
	...
	err = db.Close()
	if err != nil {
		log.Fatalf("server_test.TestMain: db.Close error: %s", err)
	}
