...
func TestEpisodeInvalidate(t *testing.T) {
	...
	episodeBeforeInvalidation := episode
	...
	invalidatedEpisode, err := r.EpisodeGet(
		ctx,
		series.ID,
		seasonNumber,
		episodeNumber,
	...
	require.NoError(err)
	require.Equal(
		null.StringFrom(invalidation),
		invalidatedEpisode.Invalidation,
	)

	// check invalidated episode audited
	...
	testutils.SetTimeLocation(
		&episodeBeforeInvalidation.DateReleased,
		audits[0].DateReleased.Location(),
	)

	require.Equal(
		episodeBeforeInvalidation,
		&models.Film{
			ID:            audits[0].ID,
			Title:         audits[0].Title,
			Descriptions:  audits[0].Descriptions,
...
func TestEpisodesInvalidateAllBySeries(t *testing.T) {
	require := require.New(t)
	...
	invalidation := "invalidation"
	err = r.EpisodesInvalidateAllBySeries(
		ctx,
		series.ID,
		user.ID,
		invalidation,
