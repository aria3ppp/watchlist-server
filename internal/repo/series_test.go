...
func TestSeriesInvalidate(t *testing.T) {
	...
	err = r.SeriesInvalidate(ctx, series.ID, user.ID, invalidation)
	require.Equal(repo.ErrNoRecord, err)
	...
	seriesBeforeInvalidation := series
	...
	invalidatedSeries, err := r.SeriesGet(ctx, series.ID)
	require.NoError(err)
	require.Equal(null.StringFrom(invalidation), invalidatedSeries.Invalidation)

	// check invalidated series audited
	...
	testutils.SetTimeLocation(
		&seriesBeforeInvalidation.DateStarted,
		audits[0].DateStarted.Location(),
	)
	testutils.SetTimeLocation(
		&seriesBeforeInvalidation.DateEnded.Time,
		audits[0].DateEnded.Time.Location(),
	)

	require.Equal(
		seriesBeforeInvalidation,
		&models.Series{
			ID:            audits[0].ID,
			Title:         audits[0].Title,
			Descriptions:  audits[0].Descriptions,
