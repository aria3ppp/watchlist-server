...
func setup() (teardown func()) {
	...
	if err != nil {
		log.Fatalf("repo_test.setup: postgres.WithInstance error: %s", err)
	}
	...
	err = migrator.Up()
	if err != nil {
		log.Fatalf("repo_test.setup: migrator.Up error: %s", err)
	}
	// create teardown
	return func() {
		// drop migrations
		err = migrator.Drop()
		if err != nil {
			log.Fatalf("repo_test.teardown: migrator.Drop error: %s", err)
		}
	}
}
...
func TestMain(m *testing.M) {
	...
	err = db.Close()
	if err != nil {
		log.Fatalf("repo_test.TestMain: db.Close error: %s", err)
	}
